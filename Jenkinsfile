pipeline {
    agent any // 어떤 에이전트(실행 서버)에서는 실행 가능

    tools { // tools : jenkins tools 에 등록된 도구 사용
    ...
    environment {
        ...
        REMOTE_HOST = "13.209.193.191" // 원격(spring) 서버 public ip

        REMOTE_DIR = "/home/ec2-user/deploy" // 원격 서버 배포 경로
        SSH_CREDENTIALS_ID = "9469df71-7021-438b-beb3-ecec39ab678b" // RSA Credentials
    }
